 ...
 */
public abstract class AScreenField extends ScreenFieldViewAdapter
    implements ActionListener, FocusListener, TableCellRenderer, TableCellEditor, KeyListener, MouseListener
{
     ...
     */
    public AScreenField(ScreenField model, boolean bEditableControl)
    {
        this();
...
//            itsMaxChars = ScreenConstants.MAX_DESC_LENGTH + 2;
        ...
        boxSize.height = ((AScreenField)this.getScreenField().getBasePanel().getScreenFieldView()).getScreenInfo().getBoxHeight(this.getScreenField().getBasePanel(), itsMaxLines);
    ...
    public Dimension getTextExtent(String tempString)
    {
        ...
        FontMetrics fm = ((AScreenField)this.getScreenField().getBasePanel().getScreenFieldView()).getScreenInfo().getFontMetrics();
        if (fm != null)
            textSize = new Dimension(fm.stringWidth(tempString), fm.getHeight());
        else
            textSize = new Dimension(((AScreenField)this.getScreenField().getParentScreen().getScreenFieldView()).getScreenInfo().getBoxWidth(tempString.length()), ((AScreenField)this.getScreenField().getBasePanel().getScreenFieldView()).getScreenInfo().getBoxHeight(this.getScreenField().getBasePanel(), 1));
        return textSize;
    }
    /**
    ...
    public ScreenInfo getScreenInfo()
    {
        BasePanel parentScreen = this.getScreenField().getParentScreen();
        if (parentScreen != null)
            return ((AScreenField)parentScreen.getScreenFieldView()).getScreenInfo();
        else
            return null;
    }
    /**
    ...
    public boolean shouldSelectCell(EventObject anEvent)
  {
    ...
    public void requestFocus()
    {
        ...
        if (this.getControl() != null)
        {
            if (this.getScreenField().getParentScreen().getScreenFieldView() instanceof AGridScreen)
            {
                ((AGridScreen)((GridScreen)this.getScreenField().getParentScreen()).getScreenFieldView()).requestFocus(this.getScreenField()); // Special tab listener for grid screens
                return;
            }
            else
            {
    ...
    {
        return ((AAppletScreen)this.getScreenField().getParentScreen().getAppletScreen().getScreenFieldView()).loadImageIcon(filename, description);
    }
    /**
    ...
    public int validateCurrentFocus()
    {
        ...
        if (component != null)
        {
            ScreenField sField = AScreenField.getComponentModel(component);
            if (sField != null)
            {
                ...
                if (sField.getConverter() != null)
                    if (sField.getScreenFieldView().getControl() == null)
                        if (sField.getParentScreen() instanceof BaseGridScreen)
                            if (sField.getScreenFieldView() instanceof AScreenField)    // Always
                {
                    ((AScreenField)sField.getScreenFieldView()).stopCellEditing();
//                    ((ABaseGridScreen)sField.getParentScreen().getScreenFieldView()).finalizeThisScreen();
                }
            }
        }
        return iErrorCode;
    ...
    public static ScreenField getComponentModel(Component aComponent)
    {
        ...
        if (listeners != null)
        {
            for (int i = 0; i < listeners.length; i++)
            {
                ...
                if (listener instanceof AScreenField)
                    return (ScreenField)((AScreenField)listener).getScreenField();
            }
        }
