 ...
 */
public class AGridScreen extends ABaseGridTableScreen
    implements TableColumnModelListener
{
     ...
     */
    public AGridScreen(ScreenField model, boolean bEditableControl)
    {
        this();
    ...
    {   // Create the table
        ...
        for (int iIndex = 0; iIndex < gridScreen.getSFieldCount(); iIndex++)
        {
            ...
            Rectangle rect = ((AScreenField)sField.getScreenFieldView()).getControlExtent();
            if (rect == null)
                rect = ((AScreenField)sField.getScreenFieldView()).calcBoxShape(new Point(0, 0));
            iMaxHeight = Math.max(iMaxHeight, rect.height);
            TableColumn newColumn = new TableColumn(iIndex, rect.width, (AScreenField)sField.getScreenFieldView(), (AScreenField)sField.getScreenFieldView());
     ...
     * NOTE: The only component that would be hooked to a AGridScreen is a sort column value.
