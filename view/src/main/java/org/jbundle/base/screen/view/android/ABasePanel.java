...
import org.jbundle.base.screen.control.android.util.ScreenLayout;
 ...
 */
public class ABasePanel extends AScreenField
{
    /**
     ...
     */
    public ABasePanel(ScreenField model, boolean bEditableControl)
    {
        this();
    ...
    public void free()
    {
        ...
        if (m_ScreenLayout != null)
            m_ScreenLayout.free((ABasePanel)this);
        super.free();
        m_ScreenLayout = null;
    }
    /**
    ...
    {   // This code is the same as the code in BasePanel.Create (Eventually use the same code)
        ...
        {   // Alternate layout, calculate the size manually
            ...
            for (int i = 0; i < ((BasePanel)this.getScreenField()).getSFieldCount(); i++)
            {
                ...
                Rectangle rectField = ((AScreenField)sField.getScreenFieldView()).getControlExtent();
                if (rectField == null)
                    rectField = ((AScreenField)sField.getScreenFieldView()).calcBoxShape(ptLoc);
                else
                {
                    ptLoc.x = rectField.x;
                    ptLoc.y = rectField.y;
